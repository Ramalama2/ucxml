...
if (isset($_POST['action']) || isset($_GET['submit_delete']) || isset($_GET['view_contact']))
{
	//User wants to save, cancel, or delete object

	$myAction = defang_input($_POST['action']);
	if ($myAction == "edit" || $_GET['submit_delete'] == 'yes' || $_GET['view_contact'] = 'yes')
	{
		if (isset($_POST['submit_save']))
		{
			...
			//delete_contact($tmp_id_contact);
			header("Location: index.php?module=view_contacts");
		} elseif ($_GET['view_contact'] == 'yes')
		{
				output_view_contact($tmp_id_contact);
...
function output_view_contact ($myID_contact)
{
	...
	$xtpl->parse('view_contact');
	$xtpl->out("view_contact");
}
